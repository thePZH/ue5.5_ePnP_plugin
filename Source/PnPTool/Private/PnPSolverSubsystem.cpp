...
void UPnPSolverSubsystem::AddPointPair(FVector ObjectPoint, FVector2D ImagePoint)
{
	m_ObjectPoints.Add(ObjectPoint);
	m_ImagePoints.Add(FVector2f(ImagePoint.X, ImagePoint.Y));
...
void UPnPSolverSubsystem::ClearAllPoints()
{
...
void UPnPSolverSubsystem::RunFullPipeline()
{
...
void UPnPSolverSubsystem::GeneratePointsByRaycast()
{
	...
	for (int32 r = 0; r <= Rows; ++r)
	{
		...
		for (int32 c = 0; c <= Cols; ++c)
		{
			...
			TraceParams.AddIgnoredActor(CamActor);
...
void UPnPSolverSubsystem::GetIntrinsicsFromCamera()
{
	if (!m_SourceCameraActor.IsValid())
	{
...
void UPnPSolverSubsystem::DrawEditorDebug()
{
	if (GetWorld() == nullptr)
	{
		return;
